...

{
    ...
    private _descriptor = [_pos, _size, _alpha, _text, _color];
    ...
    if ((_previous select 1) isNotEqualTo _size) then {
        _markerId setMarkerSizeLocal _size;
        _commands = _commands + 1;
        _markerChanged = true;
    };
    if ((_previous select 2) != _alpha) then {
        _markerId setMarkerAlphaLocal _alpha;
        _commands = _commands + 1;
        _markerChanged = true;
    };
    if ((_previous select 3) != _text) then {
        _markerId setMarkerTextLocal _text;
        _commands = _commands + 1;
        _markerChanged = true;
    };
    if ((_previous select 4) != _color) then {
        _markerId setMarkerColorLocal _color;
        _commands = _commands + 1;
        _markerChanged = true;
    };
