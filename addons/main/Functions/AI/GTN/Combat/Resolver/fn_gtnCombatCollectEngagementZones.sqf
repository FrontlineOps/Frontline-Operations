...

{
    ...
    for "_xCell" from (_seedCellX - _threatCellRadius) to (_seedCellX + _threatCellRadius) do {
        for "_yCell" from (_seedCellY - _threatCellRadius) to (_seedCellY + _threatCellRadius) do {
...

            {
                ...
                if ((_otherData get "side") isNotEqualTo _opponentSide) then { continue };
    ...
    if (_eastCandidates isEqualTo [] || {_westCandidates isEqualTo []}) then { continue };
    ...
    if (_eastRefs isEqualTo [] || {_westRefs isEqualTo []}) then { continue };
