...
while {FLO_GTN_RadarDataLinkRunning} do {
    ...
    private _groups = FLO_virtualGroups get "_groups";
    {
        ...
        private _sideKey = ["EAST", "WEST"] select ((_groupData get "side") isEqualTo west);
...

    {
        ...
        private _radars = _y;
        if (_radars isEqualTo []) then { continue };
        ...
        if (_detectedAircraft isEqualTo []) then { continue };
        ...
        if (_friendlyLeaders isEqualTo []) then { continue };
