...

{
    ...
    private _rounds = [6, 8] select (((_report get "missionCount") >= 3) || {_exposure >= (_threshold + 3)});
    ...
    if (_currentBest isEqualTo [] || {_score > (_currentBest select 0)}) then {
...

{
    ...
    if (_availableArtillery isEqualTo []) then { continue };
