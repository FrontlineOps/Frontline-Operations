...
private _candidateIds = switch (_order) do {
    case "ATTACK": {
        if (_attackCandidateIds isNotEqualTo []) then { _attackCandidateIds } else { keys _engagementGroups }
    };
    case "DEFEND": {
...

{
    ...
    if (_evaluation isEqualTo []) then { continue };
    ...
    if (_score > _bestScore) then {
        ...
        private _targetObjective = switch (_order) do {
            ...
            default {
                if (_objectiveIds isNotEqualTo []) then { _objectiveIds select 0 } else { "" };
            };
        };
