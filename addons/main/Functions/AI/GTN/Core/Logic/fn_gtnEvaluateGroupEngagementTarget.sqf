...
private _playerBonus = [0, 12] select (_isPlayerControlled);
...
switch (_order) do {
    case "ATTACK": {
        ...
        private _objectiveBonus = [0, 28] select (_inAttackObjective);
        private _reason = if (_inAttackObjective) then {
            "ATTACK_OBJECTIVE"
        } else {
            ["ATTACK_LOCAL", "ATTACK_ROUTE"] select (_routeDist <= _directDist);
        };
