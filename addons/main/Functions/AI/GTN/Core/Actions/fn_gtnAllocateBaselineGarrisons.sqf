...

{
    ...
    private _ranked = [];
    {
        ...
        _ranked pushBack [
            [1, 0] select (_gData get "inCombat"),
...
if (_available isEqualTo []) exitWith { _metrics };
...
private _candidateObjectives = [];
{
    ...
    private _pressure = (_enemyLinkedCount * 2)
        + ([0, 4] select (_underAttack))
        + ([0, 2] select (_contested));
...
while {_continueAllocation && {_available isNotEqualTo []}} do {
