...
FLO_GTN_CapabilityAnalyzer = createHashMapObject [[
    ...
    ["_analyzeUnit", {
        ...
        if ("NVGoggles" in _items || {(_items findIf { _x find "NVG" >= 0 }) isNotEqualTo -1}) then {
            (_analysis get "equipment") pushBack "NVG";
            (_analysis get "capabilities") pushBack "NIGHT_OPS";
        };
        if ((_items findIf { _x find "Laserdesignator" >= 0 }) isNotEqualTo -1) then {
            (_analysis get "equipment") pushBack "LASER_DESIGNATOR";
            (_analysis get "capabilities") pushBack "JTAC";
        };
        if ((_items findIf { _x find "Rangefinder" >= 0 }) isNotEqualTo -1) then {
            (_analysis get "equipment") pushBack "RANGEFINDER";
        };
        if ((_items findIf { _x find "UAVTerminal" >= 0 }) isNotEqualTo -1) then {
    ...
    // Analyze a collection of observed objects (e.g., from nearTargets)
    ["_analyzeObservedForces", {
        params ["_observedObjects"];
        ...
        
        {
            ...
            private _analysis = nil;
            private _isMan = _obj isKindOf "Man";
            
            if (_isMan) then {
                _analysis = _self call ["_analyzeUnit", [_obj]];
                if (!isNil "_analysis") then {
                     _unitCount = _unitCount + 1;
                };
            } else {
                _analysis = _self call ["_analyzeVehicle", [_obj]];
            };
            
            if (!isNil "_analysis") then {
                _totalPower = _totalPower + (_analysis get "combatPower");
                ...
                if (!_isMan) then {
                    ...
                    private _vType = _analysis get "vehicleType";
                    if (_vType in ["MBT", "IFV", "APC"]) then { _hasArmor = true };
                    if (_vType in ["STATIC_AT", "STATIC_AA", "STATIC_MG"]) then { _hasStatic = true };
                    if (_vType == "STATIC_AA") then { _hasAA = true };
                } else {
                    if ("AA" in _caps) then { _hasAA = true };
                };
            };
        } forEach _observedObjects;
        
        // Posture calculation
        private _posture = switch true do {
            case (_totalPower > 300): { "HEAVY" };
            case (_totalPower > 100): { "MEDIUM" };
            case (_totalPower > 0): { "LIGHT" };
            default { "NONE" };
        };
        
        createHashMapFromArray [
            ["confirmedStrength", ceil (_unitCount / 4) max 1], // Estimate groups
            ...
            ["vehicleCount", count _detectedUnits - _unitCount]
        ]
    }],
    ...
    ["_canExecuteMission", {
        ...
        switch (toUpper _missionType) do {
            case "CAS": {
                ...
                if ((_result get "availableAssets") isNotEqualTo []) then {
