/* Resolves the nearest valid ground-assault anchor inside an objective. */
params [
    ["_objectiveId", "", [""]],
...
if (_objectiveId == "") then {
    throw "FLO_fnc_campaignResolveAssaultLandAnchor: objective ID is required";
};

private _center = +(_objective get "position");
if ((count _center) < 2) then {
    throw format ["Objective %1 has an invalid assault position %2", _objectiveId, _center];
};
if ((count _center) == 2) then {
    _center pushBack 0;
} else {
...
if (_radius <= 0) then {
    throw format ["Objective %1 has invalid assault radius %2", _objectiveId, _radius];
};
