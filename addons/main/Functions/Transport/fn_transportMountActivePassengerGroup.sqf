...
if (_transportVehicles isEqualTo []) exitWith {
...
if (_aliveUnits isEqualTo []) exitWith { true };
...
if (_seatFailure || {_mountedCount != count _aliveUnits}) exitWith {
    {
        ...
        if (_vehicle != _x && {_vehicle in _transportVehicles}) then {
            unassignVehicle _x;
            moveOut _x;
        };
    } forEach _aliveUnits;
...
if (([_passengerData] call FLO_fnc_virtualizationGetTransportAttachment) == _transportGroupId) then {
