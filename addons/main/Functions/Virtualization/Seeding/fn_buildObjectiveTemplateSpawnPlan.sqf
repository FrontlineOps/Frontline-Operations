...
private _useSubtypeFilter = (keys _subtypeFilter) isNotEqualTo [];
...
private _useGroupTypeFilter = (keys _groupTypeFilter) isNotEqualTo [];
...

{
    ...
    if ((keys _desiredGroups) isEqualTo []) then { continue };
