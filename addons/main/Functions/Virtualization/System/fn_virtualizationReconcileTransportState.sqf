...

{
    ...
    if (_existingPassengers isNotEqualTo _passengerIds) then {
        [_carrierData, _passengerIds] call FLO_fnc_virtualizationSetTransportPassengers;
        _repairCount = _repairCount + 1;
    };

    if (_passengerIds isEqualTo [] && {(_carrierData get "dismountAtWaypoint") >= 0 || {(_carrierData get "transportInsertMode") != ""}}) then {
        ["VIRTUALIZATION", 1, format [
