...
private _orphanGroups = [];
{
    ...
    if ((_units findIf { isPlayer _x }) isNotEqualTo -1) then { continue };
    ...
    private _realGroup = _groupData get "realGroup";
    if (_realGroup isNotEqualTo _group) then {
        _orphanGroups pushBack [
            str _group,
            "real_group_mismatch",
            side _group,
...
private _orphanVehicles = [];
{
    ...
    private _groupData = _groups get _trackedId;
    if ((_groupData get "realGroup") isNotEqualTo _crewGroup) then {
        _orphanVehicles pushBack [
            typeOf _veh,
