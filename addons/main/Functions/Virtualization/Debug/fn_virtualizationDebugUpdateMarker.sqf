...
private _markerColor = if (_missionLock != "") then {
    ...
    switch (_state) do {
        ...
        default {
            ["ColorBlack", "ColorWhite"] select (_groupType in ["civilian", "civilianVehicle"]);
        };
    };
};
...
_markerName setMarkerAlphaLocal ([0.5, 1] select (_isActive));
...
private _text = if (_statusFlags isNotEqualTo []) then {
