...
if (_groupType isEqualTo "infantry") then {
    ...
    if (_groupsPool isEqualType [] && {_groupsPool isNotEqualTo []}) then {
...
for "_i" from 1 to _remainingGroups do {
    ...
    if (_linkAngles isNotEqualTo [] && {random 1 > 0.3}) then {
    ...
    if (_groupType in ["motorized", "mechanized"]) then {
        private _dismountGroupIds = [_groupId, true] call FLO_fnc_virtualizationCreateOrganicPackageDismount;
        {
