...
private _role = if ((keys _groupData) isNotEqualTo []) then {
...
private _memoryType = if ((keys _memory) isNotEqualTo []) then { _memory get "reportType" } else { "" };

private _securityObjectiveId = _objectiveId;
if ((keys _memory) isNotEqualTo []) then {
    _securityObjectiveId = _memory get "sourceObjectiveId";
} else {
    if (_enemyLinkedObjectives isNotEqualTo []) then {
...
private _frontlinePressure = (_objective get "contested") || {_enemyLinkedObjectives isNotEqualTo []};
...
if (_enemyLinkedObjectives isEqualTo []) then { _repairScore = _repairScore + 0.2; };
...
if (_enemyLinkedObjectives isNotEqualTo []) then { _checkpointScore = _checkpointScore + 0.2; };
...
private _minimumScore = [0.45, 0.55] select (_disposition == "WARY");
private _viableOffers = _offers select { (_x get "score") >= _minimumScore };
if (_viableOffers isEqualTo []) exitWith { _offer };
