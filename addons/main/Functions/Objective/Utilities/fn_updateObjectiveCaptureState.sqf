...
private _westDominates = _bluforCount > _opforCount && {_bluforCount >= _minUnitsToCapture} && {(_owner isNotEqualTo west)};
private _eastDominates = _opforCount > _bluforCount && {_opforCount >= _minUnitsToCapture} && {(_owner isNotEqualTo east)};
...
if (_attacker isEqualTo sideUnknown) then {
    ...
    private _attackerCount = [_opforCount, _bluforCount] select (_attacker isEqualTo west);
    private _defenderCount = [_bluforCount, _opforCount] select (_attacker isEqualTo west);
    ...
    if (_defenderCount > 0) then {
        _state = ["contested", "clearing"] select (_attackerCount > _defenderCount);
        _secureStartedAt = -1;
        _secureProgress = 0;
    } else {
...
private _stateChanged = _state != _previousState || {(_captureSide isNotEqualTo _previousCaptureSide)};
