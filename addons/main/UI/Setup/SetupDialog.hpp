...
#define FACTION_CONTENT_Y           (FACTION_PAD_Y)
#define FACTION_FULL_X              (FACTION_PAD_X)
#define FACTION_FULL_W              (FACTION_FORM_W - (2 * FACTION_PAD_X))
...
class FLO_FactionSelectDialog
{
	...
	class Controls
	{
		...
		class CloseButton: FLO_RscButton_Close
		{
			...
			y = FLO_Q(FACTION_DIALOG_Y);
			w = FLO_Q(FLO_UI_CLOSE_BTN_W);
			h = FLO_Q(FACTION_HEADER_H);
			action = "closeDialog 0";
		};

		class SetupContent: FLO_RscControlsGroup
		{
			idc = FLO_IDC_FACTION_CONTENT;
			x = FLO_Q(FACTION_DIALOG_X);
			y = FLO_Q(FACTION_DIALOG_Y + FACTION_HEADER_H);
			w = FLO_Q(FACTION_DIALOG_W);
