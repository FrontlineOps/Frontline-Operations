...
if (_updateInterval <= ((_tempoMultiplierAnchors select 0) select 0)) then {
    ...
    if (!_resolved) then {
        private _lower = _tempoMultiplierAnchors select ((count _tempoMultiplierAnchors) - 2);
        private _upper = _tempoMultiplierAnchors select ((count _tempoMultiplierAnchors) - 1);
        private _lowerTempo = _lower select 0;
        private _upperTempo = _upper select 0;
