...
private _restoredCount = 0;
{
    ...
    if ((count _mineObjects) == 0) then { continue };

    private _markerName = format ["FLO_MINEFIELD_%1", _fieldId];
    private _markerColor = if (_sideKey isEqualTo "WEST") then { "ColorWEST" } else { "ColorEAST" };
    ...
    _marker setMarkerAlpha (FLO_MinefieldConfig get "markerAlpha");
    _marker setMarkerDir ((_x get "facingDir") + 90);
    _marker setMarkerPos _fieldCenterPos;
    _marker setMarkerSize [_frontageHalfWidth, _depthHalfWidth];
