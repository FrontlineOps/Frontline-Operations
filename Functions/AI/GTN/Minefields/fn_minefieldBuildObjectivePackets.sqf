...

{
    ...
    private _packetAnchorPos = _anchorPos getPos [
        abs _offset,
        _facingDir + (if (_offset >= 0) then { 90 } else { 270 })
    ];
    ...
    private _packetLayers = ((2 + floor random 2) + (if (_subtype in ["capital", "city"]) then { 1 } else { 0 })) min (_maxLayers max 2);
...
{
    if (_roadPacketsPlaced >= (if (_allowAT) then { 2 } else { 1 })) exitWith {};
