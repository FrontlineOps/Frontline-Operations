...
FLO_fnc_addCratePurchaseActions = {
    ...
    // Add main action
    private _mainAction = _object addAction [
    ...
    // Add individual crate options as nested actions
    {
        ...
        private _crateAction = _object addAction [
            format [" - %1 (%2$)", _name, _cost],
            {
                ...
                if (_currentFunds >= _cost) then {
                    ...
                    // Add action to the crate to make it moveable
                    ...
                    hint format ["Not enough funds to purchase %1.\nRequired: %2$\nAvailable: %3$", _name, _cost, _currentFunds];
                };
            },
            _x,
            1.4,
...
FLO_fnc_initCrateSystem = {
    // Initialize on existing FOBs/OPs
    {
    ...
    // Add command for adding funds (admin only)
    if (hasInterface) then {
        FLO_fnc_addFunds = {
            params [["_amount", 0]];
            
            if (_amount > 0) then {
                private _newFunds = [_amount] call FLO_fnc_updateFunds;
                hint format ["Added %1$ to team funds.\nTotal funds: %2$", _amount, _newFunds];
            };
        };
        ...
        if (serverCommandAvailable "#kick") then {
            player addAction [
                "<img image='\A3\ui_f\data\igui\cfg\simpleTasks\types\money_ca.paa' /> Add 100$ (Admin)",
                {
                    [100] call FLO_fnc_addFunds;
                },
                nil,
                0.5,
                false,
