...
private _enemyHeavy = _enemyCatalog getOrDefault ["groundHeavy", if (!isNil "East_Ground_Vehicles_Heavy") then { East_Ground_Vehicles_Heavy } else { _enemyLight }];
...
private _template = createHashMapFromArray [
    ...
    ["fnc_setup", {
        ...
        // Find enemy-controlled objectives only
    ...
    ["fnc_spawn", {
        ...
        // Find enemy-controlled destination
        private _destObjId = [4000, getPos player, _enemySide] call FLO_fnc_getObjectiveNearPlayer;
        if (_destObjId == "") then {
        ...
        if (_destObjId == "") exitWith {
            ConVLocc = 0;
            ["SIDEMISSION", 1, format["HVT Convoy %1: No enemy objective available for destination", _missionId]] call FLO_fnc_log;
        };
        ...
        private _vehTypes = [
            ...
            selectRandom _enemyLight,
            selectRandom _enemyTransport,  // HVT vehicle
            selectRandom _enemyLight,
            selectRandom _enemyLight
        ];
        
        if (_aggrScore > 5) then {
            _vehTypes append [selectRandom _enemyHeavy];
        };
        ...
        [_startPos, _endPos, _convoyPathCallback, [_vehicles, _endPos, _data], false, "SIDEMISSION_HVT_CONVOY"] call FLO_fnc_findRoadPath;
