...
switch (toUpper _mode) do {
    ...
    case "INTERROGATE": {
        ...
        private _objectiveId = if ((count (keys _groupData)) > 0) then { _groupData get "civilianObjective" } else { _record get "objectiveId" };
        private _role = if ((count (keys _groupData)) > 0) then { _groupData get "civilianRole" } else { _unit getVariable ["FLO_CivilianRole", "resident"] };
        ...
        private _package = if ((count (keys _memory)) > 0) then {
        ...
        if ((random 1) > _cooperateChance || {(count (keys _package)) == 0}) exitWith {
