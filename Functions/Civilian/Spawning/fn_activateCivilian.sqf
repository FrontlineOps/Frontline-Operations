...

{
    ...
    _x allowFleeing 1;
    _x setBehaviour (if (_civilianRole in ["watcher", "resident"]) then { "SAFE" } else { "CARELESS" });
    _x setSpeedMode (if (_civilianRole in ["worker", "wanderer", "driver"]) then { "LIMITED" } else { "NORMAL" });
