...
if (_pressure) then {
    ...
    if (_isBuildingGroup) then {
        _targetState = if (_role == "watcher") then { "observe" } else { "home" };
...
if (_homeAnchor isEqualTo [0, 0, 0] && {(count _homePois) > 0}) then {
...
private _anchorPool = switch (_targetState) do {
    ...
    case "work": { if (_isVehicleGroup) then { _parkingPois } else { _workPois } };
...
if (_targetState in ["market", "work", "observe"]) then {
    ...
    if ((count _secondaryPool) > 0) then {
...
private _wpFormation = if (_isVehicleGroup) then { "COLUMN" } else { "FILE" };
private _wpCombatMode = "WHITE";
private _wpRadius = if (_isVehicleGroup) then { 20 } else { 6 };

if (_targetState == "shelter") then {
    _wpBehavior = "CARELESS";
    _wpSpeed = if (_isVehicleGroup) then { "NORMAL" } else { "FULL" };
    _wpRadius = if (_isVehicleGroup) then { 16 } else { 4 };
};
...
if (_isBuildingGroup) then {
    ...
    switch (_targetState) do {
        ...
        default {
            ...
            if !(_secondaryAnchor isEqualTo []) then {
            ...
            private _cyclePos = if !(_secondaryAnchor isEqualTo []) then { _secondaryAnchor } else { _targetAnchor };
