private _thisFactoryTrigger = _this select 0; 
private _AGGRSCORE = FLO_DifficultyHandle get "value";

sleep 3;
private _objectLoc = nearestobjects [getPos _thisFactoryTrigger, ["O_MBT_02_cannon_F"], 100]; 
{  
    _x hideObject true; 
    sleep 1;
    ...
    private _NewVeh = createVehicle [selectRandom East_Ground_Vehicles_Heavy, [0,0, (500 + random 2000)], [], 0, "CAN_COLLIDE"];
    _NewVeh setDir _dir;
...
private _trg = createTrigger ["EmptyDetector", getpos _thisFactoryTrigger, false];  
...
_trg setTriggerStatements [  
    ...
    _TFOBB setTriggerStatements [
        ...
        '[
            playerSide, 
            ''HQ''
        ] commandChat ''all Forces Fall Back. We Lost the Region,...'';
...
_trg setTriggerStatements [
    ...
    "{
        deleteVehicle _x
    } forEach (allUnits select {side _x == east && getPos _x distance thisTrigger < 200}); 
